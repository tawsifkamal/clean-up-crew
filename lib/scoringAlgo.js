...
export default function ScoringAlgo(issueLocation, likeLocations, viewerLocation) {
    const closeness = kCloseness * likeLocations.map(location => StrictlyDecreasingFx(getDistance(location, issueLocation), 7, 1, 1)).reduce((a, b) => a + b)
    const population = kPopulation * likeLocations.length;
    const relevance = kRelevance * StrictlyDecreasingFx(getDistance(issueLocation, viewerLocation), 4, 1, 0.4)
